            ...
            <div class="space-y-4">
                ...
                <div v-if="isEditing">
                    <div>
                        ...
                        <input v-model="editableItem.title" id="title" class="w-full p-2 border rounded-md"
                            type="text" />
                    </div>
                    <div>
                        <label for="author" class="block text-sm font-medium text-gray-700">Szerző</label>
                        <input v-model="editableItem.author" id="author" class="w-full p-2 border rounded-md"
                            type="text" />
                    </div>
                    <div>
                        <label for="published_year" class="block text-sm font-medium text-gray-700">Kiadás
                            dátuma</label>
                        <input v-model="editableItem.published_year" id="published_year"
                    ...
                    <div>
                        <label for="inventory_number" class="block text-sm font-medium text-gray-700">Leltári
                            szám</label>
                        <input v-model="editableItem.inventory_number" id="inventory_number"
                            class="w-full p-2 border rounded-md" type="text" />
                    </div>
                    <div>
                        <label for="year_of_purchasing" class="block text-sm font-medium text-gray-700">Beszerzés
                            éve</label>
                    ...
                    <div>
                        ...
                        <input v-model="editableItem.supplier" id="supplier" class="w-full p-2 border rounded-md"
                            type="text" />
                    </div>
                    <div>
...
export default {
    ...
    methods: {
        ...
        // Kölcsönzés
        borrowItem() {
            // Az item.id-t tömbként kell átadni
            axios.post(`/api/borrowings`, {
                itemIds: [this.item.id], // Egyetlen tétel id-ját tömbként adunk át
                userId: this.auth.user.id
            })
                .then(() => {
                    this.$inertia.visit('/borrowed_media'); // Átirányítás a kölcsönzött tételek oldalára
